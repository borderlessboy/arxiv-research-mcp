...
class RelevanceRanker:
    ...
    def get_feature_importance(self, papers: List[Paper], query: str) -> dict:
        ...
        try:
            ...
            # Get query vector features
            query_vector = tfidf_matrix[-1].toarray().flatten()
            
            # Get top features for the query
            feature_scores = [(feature_names[i], query_vector[i]) 
                             for i in range(len(feature_names)) if query_vector[i] > 0]
            feature_scores.sort(key=lambda x: x[1], reverse=True)
            
            return dict(feature_scores[:20])  # Top 20 features
