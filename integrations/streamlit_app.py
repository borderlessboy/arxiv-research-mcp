...
class StreamlitResearchApp:
    ...
    def __init__(self):
        self.server_path = str(Path(__file__).parent.parent / "scripts" / "run_server.py")
    ...
    def run(self):
        ...
        st.markdown("Search and analyze recent academic papers from arXiv with AI-powered relevance ranking.")
    ...
    def render_sidebar(self):
        ...
        with st.form("search_form"):
            ...
            if submitted and query:
                with st.spinner("Searching arXiv..."):
                    try:
                        results = asyncio.run(self.search_papers(
        ...
        if st.session_state.search_history:
            ...
            for i, historical_query in enumerate(reversed(st.session_state.search_history[-5:])):
    ...
    def render_papers_tab(self):
        ...
        if st.session_state.search_results:
            ...
            if selected_query:
                ...
                # Display metadata
                st.info(f"**Query:** {selected_query} | **Searched:** {timestamp.strftime('%Y-%m-%d %H:%M:%S')}")
                ...
                if papers:
                    ...
                    with col3:
                        recent_papers = len([p for p in papers if self.is_recent_paper(p.get('date', ''))])
                        st.metric("Recent Papers", recent_papers)
                    with col4:
                        with_full_text = len([p for p in papers if p.get('has_full_text', False)])
                        st.metric("With Full Text", with_full_text)
                    
                    # Display papers
                    for i, paper in enumerate(papers):
                        with st.expander(f"📄 {paper.get('title', 'Unknown Title')[:100]}...", expanded=i == 0):
                            col1, col2 = st.columns([3, 1])
                            
                            with col1:
                                st.write(f"**Authors:** {paper.get('authors', 'Unknown')}")
                                st.write(f"**Published:** {paper.get('date', 'Unknown')}")
                                st.write(f"**Categories:** {paper.get('categories', 'Unknown')}")
                            
                            with col2:
                                st.metric("Relevance", f"{paper.get('relevance_score', 0):.3f}")
                                if paper.get('url'):
                            ...
                            if paper.get('has_full_text') and paper.get('full_text'):
                                with st.expander("📖 Full Text Preview"):
                                    st.text_area(
                                        "Full Text",
                                        value=paper['full_text'][:2000] + "..." if len(paper['full_text']) > 2000 else paper['full_text'],
                                        height=200,
                                        disabled=True
                                    )
            ...
            st.info("No search results available. Use the sidebar to search for papers.")
    ...
    def render_analytics_tab(self):
        ...
        if selected_queries:
            ...
            for query in selected_queries:
                papers = self.parse_papers_from_results(st.session_state.search_results[query]['results'])
                for paper in papers:
                    paper['query'] = query
    ...
    def render_analysis_tab(self):
        ...
        if selected_query:
            ...
            analysis_type = st.selectbox(
                "Analysis Type",
                ["Key Trends", "Research Gaps", "Methodological Analysis", "Future Directions"]
            )
...
""", unsafe_allow_html=True)
    ...
    async def search_papers(self, query: str, max_results: int, years_back: int, include_full_text: bool) -> str:
    ...
    def parse_papers_from_results(self, results_text: str) -> List[Dict]:
        ...
        for section in paper_sections[1:]:  # Skip the header
            ...
            paper['categories'] = self.extract_field(section, r'\*\*Categories:\*\* ([^\n]+)')
